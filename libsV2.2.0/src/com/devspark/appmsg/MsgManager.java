...
class MsgManager extends Handler {
	...
	/**
	 * if the AppMsg already exists,update the text; else Inserts a
	 * {@link AppMsg} to be displayed.
	 * 
	 * @param appMsg
	 */
	void updateOrAdd(AppMsg appMsg) {
		if (msgQueue.isEmpty()) {
			add(appMsg);
		} else {
			AppMsg msg = msgQueue.peek();
			if (msg.getActivity() == appMsg.getActivity()) {
				msg.setText(appMsg.getText());
				reDisplayMsg();
			} else {
				removeMsg(msg);
				add(appMsg);
	...
	/**
	 * ReDisplays the next {@link AppMsg} within the queue.
	 */
	private void reDisplayMsg() {
		if (msgQueue.isEmpty()) {
			return;
		}
		// First peek whether the AppMsg is being displayed.
		...
		if (!appMsg.isShowing()) {
			...
			removeMessages(MESSAGE_REMOVE);
			msg = obtainMessage(MESSAGE_REMOVE);
			msg.obj = appMsg;
