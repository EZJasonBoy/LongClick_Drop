 ...
 * 
 * copied form 
 * https://raw.github.com/android/platform_frameworks_base/donut-release/core/java/android/widget/AbsListView.java
 * bysong@tudou.com
 */
/**
 ...
 */
public abstract class HAbsListView extends AdapterView<ListAdapter> implements TextWatcher,
        ViewTreeObserver.OnGlobalLayoutListener, Filter.FilterListener,
        ViewTreeObserver.OnTouchModeChangeListener {

	private static final String TAG = HAbsListView.class.getSimpleName();
...
//    int mMotionViewOriginalTop;
     ...
     * Acts upon click
     */
    private HAbsListView.PerformClick mPerformClick;
    ...
    public interface OnScrollListener {
         ...
         */
        public void onScroll(HAbsListView view, int firstVisibleItem, int visibleItemCount,
                int totalItemCount);
    }
...
//        a.recycle();
    ...
    public HAbsListView(Context context, AttributeSet attrs) {
        this(context, attrs, 0 /*com.android.internal.R.attr.absListViewStyle bysong*/);
    }

    public HAbsListView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        initAbsListView();

        // bysong
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.HAbsListView, defStyle, 0);

        Drawable d = a.getDrawable(R.styleable.HAbsListView_listSelector);
        if (d != null) {
            setSelector(d);
        }

        mDrawSelectorOnTop = a.getBoolean(
        		R.styleable.HAbsListView_drawSelectorOnTop, false);

//        boolean stackFromBottom = a.getBoolean(R.styleable.AbsListView_stackFromBottom, false);
//        setStackFromBottom(stackFromBottom);

        boolean scrollingCacheEnabled = a.getBoolean(R.styleable.HAbsListView_scrollingCache, true);
...
//        setTranscriptMode(transcriptMode);
        ...
        int color = a.getColor(R.styleable.HAbsListView_cacheColorHint, 0);
        setCacheColorHint(color);

        boolean enableFastScroll = a.getBoolean(R.styleable.HAbsListView_fastScrollEnabled, false);
        setFastScrollEnabled(enableFastScroll);

        boolean smoothScrollbar = a.getBoolean(R.styleable.HAbsListView_smoothScrollbar, true);
    ...
    public void setFastScrollEnabled(boolean enabled) {
        ...
        if (enabled) {
            if (mFastScroller == null) {
                mFastScroller = new FastScroller(getContext(), null/* bysong */);
            }
        } else {
            if (mFastScroller != null) {
                mFastScroller.stop();
...
//            mFastScroller.onScroll(this, mFirstPosition, getChildCount(), mItemCount);
    ...
    static class SavedState extends BaseSavedState {
        ...
        /**
         * Constructor called from {@link HAbsListView#onSaveInstanceState()}
         */
...
//        listPadding.bottom = mSelectionBottomPadding + mPaddingBottom;
     ...
     */
   //@Override
    protected boolean setFramex(int left, int top, int right, int bottom) {
        boolean changed = true;
        // bysong
//        super.setFrame(left, top, right, bottom);
        changed = ReflectUtil.view_setFrame(HAbsListView.class.getSuperclass().cast(View.class), HAbsListView.class.getSuperclass().getSuperclass().getSuperclass(), left, top, right, bottom);
...
//        final boolean clipToPadding = (mGroupFlags & CLIP_TO_PADDING_MASK) == CLIP_TO_PADDING_MASK;
        final boolean clipToPadding = (ReflectUtil.getIntField(this, getClass(), "mGroupFlags")& CLIP_TO_PADDING_MASK) == CLIP_TO_PADDING_MASK;
        if (clipToPadding) {
...
//            mGroupFlags &= ~CLIP_TO_PADDING_MASK;
            ReflectUtil.setIntField(getClass(), "mGroupFlags", ReflectUtil.getIntField(this, ViewGroup.class, "mGroupFlags") & ~CLIP_TO_PADDING_MASK);
        }
...
//            mGroupFlags |= CLIP_TO_PADDING_MASK;
    ...
    private boolean performLongPress(final View child,
        ...
        if (mOnItemLongClickListener != null) {
            handled = mOnItemLongClickListener.onItemLongClick(HAbsListView.this, child,
                    longPressPosition, longPressId);
        }
        if (!handled) {
            mContextMenuInfo = createContextMenuInfo(child, longPressPosition, longPressId);
            handled = super.showContextMenuForChild(HAbsListView.this);
        }
        if (handled) {
    ...
    public boolean showContextMenuForChild(View originalView) {
        ...
        if (longPressPosition >= 0) {
            ...
            if (mOnItemLongClickListener != null) {
                handled = mOnItemLongClickListener.onItemLongClick(HAbsListView.this, originalView,
                        longPressPosition, longPressId);
            }
            if (!handled) {
    ...
    private boolean startScrollIfNeeded(int deltaX) {
    	if (DEBUG) {
    		Log.d(TAG, "startScrollIfNeeded. deltaX: "+ deltaX);
    	}
...
//    		Log.d(TAG, "onTouchEvent. mFirstPosition: " + mFirstPosition);
    	}
        if (mFastScroller != null) {
...
//                }
                if (x != mLastX) {
                    ...
                    trackMotionScroll(deltaX, incrementalDeltaX);
...
//                        if (motionView.getTop() != mMotionViewNewTop) {
                        if (motionView.getLeft() != mMotionViewNewLeft) {
                            // We did not scroll the full amount. Treat this essentially like the
                            // start of a new touch scroll
                            final int motionPosition = findMotionRow(x);
        ...
        case MotionEvent.ACTION_UP: {
            ...
            case TOUCH_MODE_DONE_WAITING:
                ...
                if (child != null && !child.hasFocusable()) {
                    ...
                    final HAbsListView.PerformClick performClick = mPerformClick;
...
//    		Log.d(TAG, "onInterceptTouchEvent. mFirstPosition: " + mFirstPosition);
    	}
...
//                        if ((mPersistentDrawingCache & PERSISTENT_SCROLLING_CACHE) == 0) {
                        if ((ReflectUtil.getIntField(this, ViewGroup.class, "mPersistentDrawingCache")& PERSISTENT_SCROLLING_CACHE) == 0) {
                            setChildrenDrawingCacheEnabled(false);
                        }
    ...
    void trackMotionScroll(int deltaX, int incrementalDeltaX) {
    	if (DEBUG) {
    		Log.d(TAG, "trackMotionScroll. deltaX: " + deltaX + " incrementalDeltaX: " + incrementalDeltaX);
    	}
        final int childCount = getChildCount();
        if (childCount == 0) {
            return;
        ...
        final int spaceLeft = listPadding.left - firstLeft;
        final int end = getWidth() - listPadding.right;
        final int spaceRight = lastRight - end;
        ...
        if (spaceLeft >= absIncrementalDeltaX && spaceRight >= absIncrementalDeltaX) {
            hideSelector();
//            offsetChildrenTopAndBottom(incrementalDeltaY);
            ViewCompat.offsetChildrenLeftAndRight(this, incrementalDeltaX);
            
            int left = getChildAt(0).getLeft();
            Log.d(TAG, "first child left: " + left);
            
            invalidate();
//            mMotionViewNewTop = mMotionViewOriginalTop + deltaX;
            mMotionViewNewLeft = mMotionViewOriginalLeft + deltaX;
        } else {
            final int firstPosition = mFirstPosition;

            if (firstPosition == 0 && firstLeft >= listPadding.left && deltaX > 0) {
                // Don't need to move views down if the top of the first position is already visible
                return;
            }

            if (firstPosition + childCount == mItemCount && lastRight <= end && deltaX < 0) {
            ...
            if (down) {
                final int left = listPadding.left - incrementalDeltaX;
                ...
                final int right = getWidth() - listPadding.right - incrementalDeltaX;
            ...
            Log.d(TAG, "detachViewsFromParent. [" + start + ", " + count + "]");
...
//            offsetChildrenTopAndBottom(incrementalDeltaY);
            ...
            Log.d(TAG, "new firstpositon: " + mFirstPosition);
...
//        int childrenBottom = mBottom - mTop - mListPadding.bottom;
        ...
        if (toPosition >= firstPosition && toPosition < firstPosition + childCount) {
            ...
            if (selectedLeft < childrenLeft) {
            	...
            	selectedLeft = childrenBottom - selected.getMeasuredWidth()
    ...
    private void createTextFilter(boolean animateEntrance) {
        if (mPopup == null) {
            ...
            mTextFilter = (EditText) layoutInflater.inflate(
                    0/*com.android.internal.R.layout.typing_filter*/, null);
...
//        }
    ...
    public LayoutParams generateLayoutParams(AttributeSet attrs) {
        return new HAbsListView.LayoutParams(getContext(), attrs);
    }

    @Override
    protected boolean checkLayoutParams(ViewGroup.LayoutParams p) {
        return p instanceof HAbsListView.LayoutParams;
    }
    ...
    public void reclaimViews(List<View> views) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            HAbsListView.LayoutParams lp = (HAbsListView.LayoutParams) child.getLayoutParams();
...
//    }
    ...
    class RecycleBin {
        ...
        void addScrapView(View scrap) {
            HAbsListView.LayoutParams lp = (HAbsListView.LayoutParams) scrap.getLayoutParams();
            if (lp == null) {
                return;
            }
        ...
        void scrapActiveViews() {
            ...
            for (int i = 0; i < count; ++i) {
                ...
                if (victim != null) {
                    int whichScrap = ((HAbsListView.LayoutParams)
