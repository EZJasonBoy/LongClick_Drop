...
/**
 * copied from 
 * https://raw.github.com/android/platform_frameworks_base/donut-release/core/java/android/widget/ListView.java
 * 
 * bysong@tudou.com
 */
...
public class HListView extends HAbsListView {
	private static final String TAG = HListView.class.getSimpleName();
    ...
    private ArrayList<FixedViewInfo> mHeaderViewInfos = new ArrayList<FixedViewInfo>();//Lists.newArrayList();
    private ArrayList<FixedViewInfo> mFooterViewInfos = new ArrayList<FixedViewInfo>();//Lists.newArrayList();
    ...
    public HListView(Context context, AttributeSet attrs) {
        this(context, attrs, 0/*com.android.internal.R.attr.listViewStyle*/);
    }
    ...
    public HListView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                /*com.android.internal.*/R.styleable.HListView, defStyle, 0);

        CharSequence[] entries = a.getTextArray(
        		/*com.android.internal.*/R.styleable.HListView_entries);
        if (entries != null) {
        ...
        final int dividerHeight = a.getDimensionPixelSize(
        		/*com.android.internal.*/R.styleable.HListView_dividerWidth, 0);
        if (dividerHeight != 0) {
...
//            mAdapter = new HeaderViewListAdapter(mHeaderViewInfos, mFooterViewInfos, adapter);
     ...
     *         range that we draw.
     */
    private View fillDown(int pos, int nextLeft) {
     ...
     */
    private View fillUp(int pos, int nextRight) {
        View selectedView = null;
    ...
    private View fillFromMiddle(int childrenLeft, int childrenRight) {
        ...
        int selHeight = sel.getMeasuredWidth();
        if (selHeight <= width) {
            sel.offsetLeftAndRight((width - selHeight) / 2);
        }
    ...
    private View fillFromSelection(int selectedTop, int childrenTop, int childrenBottom) {
        ...
        sel = makeAndAddView(selectedPosition, selectedTop, true, mListPadding.top, true);


        // Some of the newly selected item extends below the bottom of the list
        if (sel.getRight() > bottomSelectionPixel) {
            // Find space available above the selection into which we can scroll
            // upwards
            final int spaceAbove = sel.getLeft() - topSelectionPixel;

            // Find space required to bring the bottom of the selected item
            // fully into view
            final int spaceBelow = sel.getRight() - bottomSelectionPixel;
            final int offset = Math.min(spaceAbove, spaceBelow);

            // Now offset the selected item to get it into view
//            sel.offsetTopAndBottom(-offset);
            sel.offsetLeftAndRight(-offset);
        } else if (sel.getLeft() < topSelectionPixel) {
            // Find space required to bring the top of the selected item fully
            // into view
            final int spaceAbove = topSelectionPixel - sel.getLeft();

            // Find space available below the selection into which we can scroll
            // downwards
            final int spaceBelow = bottomSelectionPixel - sel.getRight();
            final int offset = Math.min(spaceAbove, spaceBelow);
...
//            sel.offsetTopAndBottom(offset);
    ...
    private View moveSelection(View oldSel, View newSel, int delta, int childrenLeft,
        ...
        final int topSelectionPixel = getTopSelectionPixel(childrenLeft, fadingEdgeLength,
                selectedPosition);
        final int bottomSelectionPixel = getBottomSelectionPixel(childrenLeft, fadingEdgeLength,
                selectedPosition);
        ...
        if (delta > 0) {
            ...
            if (sel.getRight() > bottomSelectionPixel) {
                ...
                int spaceAbove = sel.getLeft() - topSelectionPixel;

                // Find space required to bring the bottom of the selected item fully into view
                int spaceBelow = sel.getRight() - bottomSelectionPixel;

                // Don't scroll more than half the height of the list
                int halfVerticalSpace = (childrenRight - childrenLeft) / 2;
...
//                sel.offsetTopAndBottom(-offset);
            ...
            // Fill in views above and below
            if (!mStackFromBottom) {
                fillUp(mSelectedPosition - 2, sel.getLeft() - dividerWidth);
                adjustViewsUpOrDown();
                fillDown(mSelectedPosition + 1, sel.getRight() + dividerWidth);
            } else {
                fillDown(mSelectedPosition + 1, sel.getRight() + dividerWidth);
                adjustViewsUpOrDown();
                fillUp(mSelectedPosition - 2, sel.getLeft() - dividerWidth);
            ...
            if (newSel != null) {
                ...
                sel = makeAndAddView(selectedPosition, newSel.getRight(), true, mListPadding.top,
                        true);
            } else {
                // If (A) was not on screen and so did not have a view, position
                // it above the oldSel (B)
                sel = makeAndAddView(selectedPosition, oldSel.getRight(), false, mListPadding.top,
                        true);
            }
            ...
            if (sel.getTop() < topSelectionPixel) {
                ...
                int spaceAbove = topSelectionPixel - sel.getLeft();

               // Find space available below the selection into which we can scroll downwards
                int spaceBelow = bottomSelectionPixel - sel.getRight();
...
//                sel.offsetTopAndBottom(offset);
            ...
            if (oldLeft < childrenLeft) {
                ...
                int newBottom = sel.getRight();
                if (newBottom < childrenLeft + 20) {
                    // Not enough visible -- bring it onscreen
//                    sel.offsetTopAndBottom(childrenLeft - sel.getTop());
                    sel.offsetLeftAndRight(childrenLeft - sel.getTop());
                }
            }
     ...
     * @param left Pixel offset from the top of this view to the top of the
     *        reference view.
     *
    ...
    private void correctTooHigh(int childCount) {
        ...
        if (lastPosition == mItemCount - 1 && childCount > 0) {
            ...
            // ... and its bottom edge
            final int lastBottom = lastChild.getRight();
            ...
            final int firstTop = firstChild.getLeft();
...
//                offsetChildrenTopAndBottom(bottomOffset);
                ViewCompat.offsetChildrenLeftAndRight(this, bottomOffset);
                if (mFirstPosition > 0) {
...
//                offsetChildrenTopAndBottom(-topOffset);
     ...
     */
    private View makeAndAddView(int position, int x, boolean flow, int childrenLeft,
            boolean selected) {
        View child;
        ...
        setupChild(child, position, x, flow, childrenLeft, selected, false);
     ...
     */
    private void setupChild(View child, int position, int x, boolean flowDown, int childrenTop,
    ...
    boolean pageScroll(int direction) {
        ...
        if (nextPage >= 0) {
            ...
            if (position >= 0) {
                mLayoutMode = LAYOUT_SPECIFIC;
                mSpecificTop = getPaddingLeft() + getHorizontalFadingEdgeLength();
    ...
    private void measureAndAdjustDown(View child, int childIndex, int numChildren) {
        int oldＷidth = child.getWidth();
        measureItem(child);
        if (child.getMeasuredWidth() != oldＷidth) {
            // lay out the view, preserving its top
            relayoutMeasuredItem(child);

            // adjust views below appropriately
            final int widthDelta = child.getMeasuredWidth() - oldＷidth;
            for (int i = childIndex + 1; i < numChildren; i++) {
//                getChildAt(i).offsetTopAndBottom(widthDelta);
    ...
    private ArrowScrollFocusResult arrowScrollFocused(final int direction) {
        ...
        if (selectedView != null && selectedView.hasFocus()) {
            ...
            if (direction == View.FOCUS_LEFT) {
                ...
                final int listLeft = mListPadding.top +
...
//        offsetChildrenTopAndBottom(amount);
    	ViewCompat.offsetChildrenLeftAndRight(this, amount);
...
//                offsetChildrenTopAndBottom(listBottom - last.getBottom());
            	ViewCompat.offsetChildrenLeftAndRight(this, listRight - last.getBottom());
            }

            // top views may be panned off screen
            View first = getChildAt(0);
            while (first.getRight() < listLeft) {
                HAbsListView.LayoutParams layoutParams = (LayoutParams) first.getLayoutParams();
...
//                offsetChildrenTopAndBottom(listTop - first.getTop());
            	ViewCompat.offsetChildrenLeftAndRight(this, listLeft - first.getTop());
            }
            ...
            while (last.getLeft() > listRight) {
                HAbsListView.LayoutParams layoutParams = (LayoutParams) last.getLayoutParams();
                if (recycleBin.shouldRecycleViewType(layoutParams.viewType)) {
    ...
    private View addViewAbove(View theView, int position) {
        ...
        setupChild(view, abovePosition, edgeOfNewChild, false, mListPadding.top, false, false);
        return view;
    }
    ...
    private View addViewBelow(View theView, int position) {
        ...
        setupChild(view, belowPosition, edgeOfNewChild, true, mListPadding.top, false, false);
        return view;
    }
     ...
     */
    //@Override
    protected View findViewTraversalX(int id) {
        View v = null;
...
//              if (!v.isRootNamespace()) {
     ...
     */
    //@Override
    protected View findViewWithTagTraversalX(Object tag) {
        View v = null;
...
//                if (!v.isRootNamespace()) {
    ...
    static class SavedState extends BaseSavedState {
        ...
        /**
         * Constructor called from {@link HListView#onSaveInstanceState()}
         */
