...
public class RadialSelectorView extends View {
	...
	public void initialize(Context context, boolean is24HourMode,
			boolean hasInnerCircle, boolean disappearsOut,
	...
	public int getDegreesFromCoords(float pointX, float pointY,
		...
		if (mHasInnerCircle) {
			if (forceLegal) {
				...
				int distanceToInnerNumber = (int) Math.abs(hypotenuse
						- innerNumberRadius);
	...
	public void onDraw(Canvas canvas) {
		...
		int pointX = mXCenter
				+ (int) (mLineLength * Math.sin(mSelectionRadians));
		int pointY = mYCenter
				- (int) (mLineLength * Math.cos(mSelectionRadians));
		...
		if (mForceDrawDot | mSelectionDegrees % 30 != 0) {
			...
			// anyway).
			mPaint.setAlpha(255);
			canvas.drawCircle(pointX, pointY, (mSelectionRadius * 2 / 7),
					mPaint);
		} else {
			...
			pointX = mXCenter
					+ (int) (lineLength * Math.sin(mSelectionRadians));
			pointY = mYCenter
					- (int) (lineLength * Math.cos(mSelectionRadians));
		}
