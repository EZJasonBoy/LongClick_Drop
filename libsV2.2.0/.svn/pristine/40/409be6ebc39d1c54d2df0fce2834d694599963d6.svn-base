...
/**
 * 带下拉刷新+header标示悬停的listview
 * 
 * @author Zxh
 * 
 */
public class PullToRefreshStickyListView extends
		PullToRefreshBase<StickyListHeadersListView> implements
		OnScrollListener {
	...
	protected StickyListHeadersListView createRefreshableView(Context context,
		...
		StickyListHeadersListView stickyListView = new StickyListHeadersListView(
				context, attrs);
		mListView = stickyListView.getWrappedList();
	...
	private boolean isFirstItemVisible() {
		...
		int mostTop = (mListView.getChildCount() > 0) ? mListView.getChildAt(0)
				.getTop() : 0;
		// zxh 2014/09/30添加判断条件mListView.getFirstVisiblePosition()==0
		// 解决带有headerview的listview的bug
