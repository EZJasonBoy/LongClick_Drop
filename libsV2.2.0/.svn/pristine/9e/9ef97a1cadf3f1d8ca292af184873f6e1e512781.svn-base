...
public class AppMsg {
	 ...
	 * @see #setDuration
	 */
	public static final int LENGTH_MID = 2000;
	 ...
	 * style.
	 */
	public static final Style STYLE_ALERT = new Style(LENGTH_SHORT,
			R.color.alert);
	 ...
	 * style.
	 */
	public static final Style STYLE_CONFIRM = new Style(LENGTH_SHORT,
			R.color.confirm);
	 ...
	 * style.
	 */
	public static final Style STYLE_INFO = new Style(LENGTH_MID, R.color.info);
	 ...
	 */
	public static AppMsg makeText(Activity context, CharSequence text,
			Style style) {
		return makeText(context, text, style, R.layout.app_msg);
	}

	/**
	 * Make a {@link AppMsg} with a custom layout. The layout must have a
	 * {@link TextView} com id {@link android.R.id.message}
	 * 
	 * @param context
	 *            The context to use. Usually your {@link android.app.Activity}
	 *            object.
	...
	/**
	 * Make a non-floating {@link AppMsg} with a custom view presented inside
	 * the layout. It can be used to create non-floating notifications if
	 * floating is false.
	 * 
	 * @param context
	 *            The context to use. Usually your {@link android.app.Activity}
	...
	/**
	 * Make a {@link AppMsg} with a custom view. It can be used to create
	 * non-floating notifications if floating is false.
	 * 
	 * @param context
	 *            The context to use. Usually your {@link android.app.Activity}
	 *            object.
	 ...
	 *            true if it'll float.
	 */
	private static AppMsg makeText(Activity context, CharSequence text,
		...
		ColorStateList csl = (ColorStateList) context.getResources()
				.getColorStateList(R.color.text_info);
		tv.setTextColor(csl);
		tv.setText(text);
	 ...
	 * resource. The layout must have a {@link TextView} com id
	...
	/**
	 * when the view is Showing,update the message
	 */
	public void showOrUpdate() {
		MsgManager manager = MsgManager.getInstance();
		manager.updateOrAdd(this);
	}
	...
	/**
	 * Get the text in a AppMsg that was previously created using one of the
	 * makeText() methods.
	 * 
	 * @return The text for the AppMsg
	 */
	...
	public LayoutParams getLayoutParams() {
		if (mLayoutParams == null) {
			int px = (int) TypedValue.applyDimension(
					TypedValue.COMPLEX_UNIT_DIP, 8, mContext.getApplication()
							.getResources().getDisplayMetrics());
			int top = (int) TypedValue.applyDimension(
					TypedValue.COMPLEX_UNIT_DIP, 56, mContext.getApplication()
							.getResources().getDisplayMetrics());
			mLayoutParams = new FrameLayout.LayoutParams(
					LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);
			mLayoutParams.setMargins(px, top, px, 0);
		}
