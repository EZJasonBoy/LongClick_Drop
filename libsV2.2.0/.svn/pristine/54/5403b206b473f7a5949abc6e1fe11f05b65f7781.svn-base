...
public class DefaultEditTextValidator implements EditTextValidator {
	...
	public DefaultEditTextValidator(EditText editText, AttributeSet attrs,
		...
		testType = typedArray.getInt(R.styleable.FormEditText_testType,
				EditTextValidator.TEST_NOCHECK);
		testErrorString = typedArray
				.getString(R.styleable.FormEditText_testErrorString);
		numericBetweenFrom = typedArray.getInt(
				R.styleable.FormEditText_numericBetweenFrom, 1);
		numericBetweenTo = typedArray.getInt(
				R.styleable.FormEditText_numericBetweenTo, 999);
	...
	public void resetValidators(Context context) {
		...
		case TEST_PERSONNAME:
			toAdd = new PersonNameValidator(
					TextUtils.isEmpty(testErrorString) ? context
							.getString(R.string.error_notvalid_personname)
							: testErrorString);
			break;
		case TEST_PERSONFULLNAME:
			toAdd = new PersonFullNameValidator(
					TextUtils.isEmpty(testErrorString) ? context
							.getString(R.string.error_notvalid_personfullname)
							: testErrorString);
			break;
		...
		case TEST_DATE:
			toAdd = new DateValidator(
					TextUtils.isEmpty(testErrorString) ? context.getString(R.string.error_date_not_valid)
							: testErrorString, customFormat);
			break;

		case TEST_NUMERICBETWEEN:
			toAdd = new NumericBetweenValidator(
					TextUtils.isEmpty(testErrorString) ? String.format(
							context.getString(R.string.error_number_between),
							numericBetweenFrom, numericBetweenTo)
							: testErrorString, numericBetweenFrom,
					numericBetweenTo);
			break;
		...
		if (!emptyAllowed) { // If the xml tells us that this is a required
								// field, we will add the EmptyValidator.
	...
	protected int numericBetweenFrom;
	protected int numericBetweenTo;
