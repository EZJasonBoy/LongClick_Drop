...
public class SwipeListViewTouchListener implements View.OnTouchListener {
	...
	private void generateRevealAnimate(final View view, final boolean swap,
		...
		if (opened.get(position)) {
			...
			if (swap) {
				moveDis = moveTo = swapRight ? (int) (viewWidth - rightOffset)
						: (int) (-viewWidth + leftOffset);
				moveTo = moveTo == viewWidth ? 0 : moveTo;
			}
		}
		final boolean flg = moveDis != viewWidth;
		if (view != null) {
	...
	public boolean onTouch(View view, MotionEvent motionEvent) {
		...
		case MotionEvent.ACTION_DOWN: {
			Log.v("SwipeListView", "ACTION_DOWN");
		...
		case MotionEvent.ACTION_MOVE: {
			if (velocityTracker == null || paused
					|| downPosition == ListView.INVALID_POSITION) {
				Log.v("SwipeListView", "ACTION_DMOVE_PAUSED");
				break;
			}
	...
	/**
	 * 检测视图的可滑动范围,防止frontView滑出屏幕
	 * 
	 * @authur xuzj
	 * @param deltaX
	 */
	private float check(float deltaX, boolean isOpened) {
		if (rightOffset != 0) {
			if (deltaX > viewWidth - rightOffset) {
			...
			if (deltaX < -viewWidth + leftOffset) {
	...
	public void move(float deltaX) {
		deltaX = check(deltaX, opened.get(downPosition));
