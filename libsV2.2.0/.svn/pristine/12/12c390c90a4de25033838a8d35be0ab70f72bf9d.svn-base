...
public class WrapperView extends ViewGroup {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		// measure header or divider. when there is a header visible it acts as
		// the divider
		if (mHeader != null) {
			ViewGroup.LayoutParams params = mHeader.getLayoutParams();
			if (params != null && params.height > 0) {
				mHeader.measure(childWidthMeasureSpec, MeasureSpec
						.makeMeasureSpec(params.height, MeasureSpec.EXACTLY));
			} else {
		...
		if (params != null && params.height > 0) {
			mItem.measure(childWidthMeasureSpec, MeasureSpec.makeMeasureSpec(
